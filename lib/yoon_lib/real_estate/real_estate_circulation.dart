...
class RealEstateCirculationPage extends StatefulWidget {
  final Function(Widget, double)
  switchPageCallback; // switchPageCallback을 생성자로 받음
  final double page_num;

  const RealEstateCirculationPage({
    Key? key,
    required this.switchPageCallback,
    required this.page_num,
  }) : super(key: key);

  State createState() => TaxCirculationPageState();
}

class TaxCirculationPageState extends State<RealEstateCirculationPage> {
  int circulation = 1;
  String page_name = '부동산등기사항증명서';
