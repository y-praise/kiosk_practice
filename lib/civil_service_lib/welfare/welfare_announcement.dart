...
import 'welfare_selection3.dart';
import 'welfare_selection.dart';
import 'welfare_selection2.dart';
import '../widgets.dart';

class WelfareAnnouncement extends StatelessWidget {
  final Function(Widget, double)
  switchPageCallback; // switchPageCallback을 생성자로 받음
  final double page_num;

  const WelfareAnnouncement({
    Key? key,
  ...
  Widget _next_page() {
    if (page_num == double.parse('3.1')) {
      return WelfareSelectionPage(
        switchPageCallback: switchPageCallback,
        page_num: page_num,
      );
    } else if (page_num == double.parse('3.2')) {
      return WelfareSelectionPage2(
        switchPageCallback: switchPageCallback,
        page_num: page_num,
      );
    } else {
      return WelfareSelectionPage3(
        switchPageCallback: switchPageCallback,
        page_num: page_num,
      );
    }
