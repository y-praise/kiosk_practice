...
import 'passport_selection3.dart';
import 'passport_selection.dart';
import 'passport_selection2.dart';
import '../widgets.dart';

class PassportAnnouncement extends StatelessWidget {
  ...
  Widget _next_page() {
    if (page_num == double.parse('12.1')) {
      return PassportSelectionPage(
        switchPageCallback: switchPageCallback,
        page_num: page_num,
      );
    } else if (page_num == double.parse('12.2')) {
      return PassportSelectionPage2(
        switchPageCallback: switchPageCallback,
        page_num: page_num,
      );
    } else {
      return PassportSelectionPage3(
        switchPageCallback: switchPageCallback,
        page_num: page_num,
      );
    }
            ...
            child: Center(
              child: Column(
                children: [
                  FittedBox(
                    fit: BoxFit.scaleDown,
                    child: Text('여권 관련 서류 발급 안내입니다.'),
                  ),
                  SizedBox(height: 20),
                  FittedBox(
                    fit: BoxFit.scaleDown,
                    child: Text(
                      '여권 정보 증명서, 여권 발급 기록 증명서 등을 발급받으실 수 있습니다.',
