...
import 'military_selection.dart';
import '../widgets.dart';

class MilitaryAnnouncement extends StatelessWidget {
  final Function(Widget, double)
  switchPageCallback; // switchPageCallback을 생성자로 받음
  final double page_num;

  const MilitaryAnnouncement({
    Key? key,
            ...
            child: Center(
              child: Column(
                children: [
                  FittedBox(
                    fit: BoxFit.scaleDown,
                    child: Text('병적증명서 발급 안내입니다.'),
                  ),
                  SizedBox(height: 20),
                  FittedBox(
                    fit: BoxFit.scaleDown,
                    child: Text(
                      '군 복무를 마쳤거나, 복무 중, 또는 면제된 사실을 증명하는 서류를 발급받으실 수 있습니다.',
